...
def main(args):
    ...
    with tf.name_scope('loss'):
        loss = tf.norm(mx, ord=2) + tf.norm(tf.transpose(centers) - centers_var, ord=2)
    ...
    logdir = r'D:\models\mnist_conv_myloss-' + datetime.strftime(datetime.now(), '%Y%m%d-%H%M%S')
    ...
    with tf.Session() as sess:
        ...
        for i in range(20001):
            ...
            # sess.run([centers], feed_dict={x: batch[0], y_: batch[1]})
            # sess.run([loss], feed_dict={x: batch[0], y_: batch[1]})
            # sess.run([centers], feed_dict={x: batch[0], y_: batch[1]})
            # sess.run([centers], feed_dict={x: batch[0], y_: batch[1]})
            # sess.run([centers], feed_dict={x: batch[0], y_: batch[1]})
        tbat = get_batch(False)
