...
def main(args):
    ...
    with tf.name_scope('classifier'):
        W_fc2 = weight_variable([features_sz, 10])
        variable_summaries(W_fc2, 'W_fc2')
        #b_fc2 = bias_variable([10])
        #variable_summaries(b_fc2, 'b_fc2')

        y_conv = tf.matmul(features, W_fc2)# + b_fc2

    with tf.name_scope('loss'):
        loss = tf.losses.sparse_softmax_cross_entropy(labels=y_, logits=y_conv)
    ...
    logdir = r'D:\models\mnist_conv_softmax-' + datetime.strftime(datetime.now(), '%Y%m%d-%H%M%S')+'_features='+str(features_sz)
