...
def main(args):
    ...
    with tf.name_scope('classifier'):
        ...
        # set_op = centers_var.assign(tf.transpose(centers))
        # ema = tf.train.ExponentialMovingAverage(decay=centers_decay)
        # with tf.control_dependencies([set_op]):
        #    centers_apply = ema.apply([centers_var])
        # centers_averaged = ema.average(centers_var)
        # variable_summaries(centers_averaged, 'centers_averaged')
        variable_summaries(centers_var, 'centers_averaged')
        variable_summaries(centers,'centers_values')
        # y_conv = tf.matmul(features, centers_averaged)
        y_conv = tf.matmul(features, centers_var)

    with tf.name_scope('loss'):
        myloss = tf.norm(mx, ord=2) + tf.norm(tf.transpose(centers) - centers_var, ord=2)
        smloss=tf.losses.sparse_softmax_cross_entropy(labels=y_, logits=y_conv)

    with tf.name_scope('optimizer'):
        # train_op=
        train_step_my = tf.train.AdamOptimizer(1e-4).minimize(myloss)
        train_step_sm = tf.train.AdamOptimizer(1e-4).minimize(smloss)
        # with tf.control_dependencies([train_op, centers_apply]):
        #    train_step = tf.no_op()
    ...
    with tf.name_scope('stats'):
        tf.summary.scalar('myloss', myloss)
        tf.summary.scalar('loss', smloss)
        tf.summary.scalar('train_accuracy', accuracy)

    logdir = r'D:\models\mnist_conv_myloss+softmax-' + datetime.strftime(datetime.now(), '%Y%m%d-%H%M%S')+'_features='+str(features_sz)
            ...
            if unordered:
                ...
                while sz < images_p_class * 10:
                    ...
                    if len(imgs[b[1][0]]) < images_p_class:
                        ...
                        sz = sz + 1
                return np.asarray(imgs).reshape([-1, 784]), np.asarray(lbls).reshape([-1])
    ...
    with tf.Session() as sess:
        ...
        for i in range(20001):
            ...
            if i<15000:
                batch = get_batch(True, True, batch_sz)
                if i % 100 == 0:
                    train_accuracy, summ = sess.run([accuracy, merged], feed_dict={
                        x: batch[0], y_: batch[1]})
                    print('step %d, training accuracy %g' % (i, train_accuracy))
                    train_writer.add_summary(summ, i)
                sess.run([train_step_sm], feed_dict={x: batch[0], y_: batch[1]})
            else:
                ...
                if i % 100 == 0:
                    ...
                    print('step %d, training accuracy %g' % (i, train_accuracy))
                    train_writer.add_summary(summ, i)
                sess.run([train_step_my], feed_dict={x: batch[0], y_: batch[1]})
        tbat = get_batch(False)
        imgs = np.split(tbat[0], 200, 0)
        lbls = np.split(tbat[1], 200, 0)
        acc = 0.
